...
} from "../components/FriendlyFeudLogo";
import {
  Users,
  Plus,
  RefreshCw,
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-semibold border ${c.cls}`}
    >
      {status === "playing" && (
...
async function checkNicknameDetailed(
  ...
  try {
    ...
    if (roomsRes.ok) {
      ...
      if (taken)
        return { taken: true, reason: "Currently in use in a live room" };
    }
    return { taken: false };
  } catch {
    return { taken: false };
...
export default function Lobby() {
  ...
  const [pendingInviteRoom, setPendingInviteRoom] = useState<string | null>(
    () => {
      ...
      if (joinRoom) {
        try {
          sessionStorage.setItem("pendingInviteJoin", joinRoom);
        } catch {
          /* ignore */
        }
  ...
  const [joinRoomPlayers, setJoinRoomPlayers] = useState<Array<{
    id: string;
    name: string;
    team: 1 | 2;
    isHost: boolean;
  ...
  const handleSoloPlay = () => {
    ...
    createSoloGame(
      ...
      (roomId) => {
        ...
        setSoloOpen(false);
        setLocation(
          `/room/${roomId}?name=${encodeURIComponent(playerName)}&team=1`,
        );
      },
      (error) => {
        setSoloLoading(false);
    ...
    async function checkSlot() {
      ...
      try {
        const res = await fetch(
          `/api/player-slots?nickname=${encodeURIComponent(nickname)}`,
        );
        if (res.ok && !cancelled) {
          const data = await res.json();
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (trimmedRoomName.length > 32) {
      setCreateError("Room name must be 32 characters or fewer.");
      return;
    }
    if (!trimmedNickname) return;
    if (trimmedNickname.length > 16) {
      setCreateError("Nickname must be 16 characters or fewer.");
      return;
    }
    if (
      !ALLOWED_TOTAL_ROUNDS.includes(
        form.totalRounds as (typeof ALLOWED_TOTAL_ROUNDS)[number],
      )
    ) {
    ...
    try {
      const { taken, reason } = await checkNicknameDetailed(
        trimmedNickname,
        lockedNickname,
      );
      if (taken) {
        setCreateError(
          reason ||
            `Nickname "${trimmedNickname}" is already in use. Change it first.`,
        );
        return;
      }
      ...
      const room = await createRoomApi({
        ...
        hostName: trimmedNickname,
      });
      localStorage.setItem("playerName", trimmedNickname);
      setCreateOpen(false);
      setLocation(
        `/room/${room.id}?name=${encodeURIComponent(trimmedNickname)}&team=1`,
      );
    } catch (err) {
  ...
  async function handleJoin(roomId: string) {
    if (!nickname.trim()) {
      ...
      try {
        sessionStorage.setItem("pendingInviteJoin", roomId);
      } catch {
        /* ignore */
      }
      requireNickname();
  ...
  async function handleChangeNickname() {
    ...
    if (trimmed.length > 16) {
      setChangeNicknameError("Nickname must be 16 characters or fewer.");
      return;
    }
    ...
    try {
      ...
      if (taken) {
        setChangeNicknameError(
          reason || `"${trimmed}" is already in use. Pick another.`,
        );
        return;
      }
      setNickname(trimmed);
  ...
  async function confirmJoin() {
    ...
    if (!trimmedNickname) return;
    if (trimmedNickname.length > 16) {
      setJoinError("Nickname must be 16 characters or fewer.");
      return;
    }
    ...
    setLocation(
      `/room/${joinRoomId}?name=${encodeURIComponent(trimmedNickname)}&team=${joinTeam}`,
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#070d1f] text-white overflow-x-hidden">
        ...
        <div className="max-w-6xl mx-auto px-3 sm:px-6 py-3 sm:py-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            >
              <RefreshCw
                ...
                className={
                  refreshSpinKey === 0
                    ? "w-4 h-4"
                    : "w-4 h-4 animate-[spin_0.65s_cubic-bezier(0.22,1,0.36,1)]"
                }
              />
            </button>
                ...
                {nickname && (
                  <>
                    {!isSignedIn && (
                      <DropdownMenuItem
                        disabled
                        className="opacity-100 focus:bg-transparent"
                      >
            ...
            <Dialog
              open={createOpen}
              onOpenChange={(v) => {
                if (v && !requireNickname("create")) return;
                setCreateOpen(v);
              ...
              <DialogContent className="bg-[#0d1525]/95 backdrop-blur-xl border border-white/10 text-white max-w-md shadow-2xl">
                ...
                <form onSubmit={handleCreate} className="space-y-4 mt-2">
                  ...
                  <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-sm text-amber-300/80">
                    Hosting as{" "}
                    <span className="font-bold text-amber-400">{nickname}</span>
                  </div>
                    ...
                    <div>
                      ...
                      >
                        {ALLOWED_TOTAL_ROUNDS.map((n) => (
                          <option key={n} value={n} className="bg-[#0d1525]">
                            {n} round{n !== 1 ? "s" : ""}
                          </option>
                        ))}
                      </select>
      ...
      <aside
        className="fixed left-4 top-32 z-20 hidden 2xl:block w-[160px]"
        aria-label="Advertisement"
      >
        <AdsterraWidget variant="banner160x600" minViewportWidth={1536} />
      </aside>
      <aside
        className="fixed right-4 top-32 z-20 hidden 2xl:block w-[160px]"
        aria-label="Advertisement"
      >
        <AdsterraWidget variant="banner160x600" minViewportWidth={1536} />
      </aside>
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-3 sm:px-6 py-6 sm:py-10">
        <div className="max-w-4xl mx-auto mb-6">
          <AdsterraWidget
            ...
            mobileBannerConfig={{
              key: "a27b4847f4b5d00d63623929539b2b8a",
              width: 320,
              height: 50,
            }}
          />
              ...
              <div className="space-y-4">
                <div className="flex bg-[#070d1f] p-1 rounded-lg border border-white/5">
                  ...
                  >
                    ...
                    <span className="text-pink-400">Custom Topic</span>
                  </button>
                </div>
                <div>
                  <Label
                  ...
                  >
                    <option value={2} className="bg-[#0d1525]">
                      2 rounds
                    </option>
                    <option value={4} className="bg-[#0d1525]">
                      4 rounds
                    </option>
                    <option value={6} className="bg-[#0d1525]">
                      6 rounds
                    </option>
                    <option value={8} className="bg-[#0d1525]">
                      8 rounds
                    </option>
                    <option value={10} className="bg-[#0d1525]">
                      10 rounds
                    </option>
                  </select>
                </div>
                {soloMode === "custom" && (
                  <div className="animate-in fade-in slide-in-from-top-2">
                    <Label
                      htmlFor="solo-topic"
                      className="text-pink-300/90 text-sm font-medium mb-2 flex items-center gap-1.5"
                    >
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 gap-4 rounded-3xl bg-white/[0.02] border border-white/5">
            ...
            <Button
              onClick={() => {
                if (requireNickname("create")) setCreateOpen(true);
              }}
                ...
                >
                  ...
                  <div className="relative p-4 sm:p-5">
                    ...
                    <p className="text-xs text-slate-500 mb-4">
                      Host:{" "}
                      <span className="text-slate-400">{room.hostName}</span>
                    </p>
                    ...
                    {isReconnectRoom ? (
                      <Button
                        ...
                        onClick={() => {
                          playClickSound();
                          setLocation(
                            `/room/${room.id}?name=${encodeURIComponent(nickname)}&team=${reconnectSlot!.team}`,
                          );
                        }}
                      >
                        ↩ Reconnect
        ...
        <DialogContent className="bg-[#0d1525]/95 backdrop-blur-xl border border-white/10 text-white max-w-sm shadow-2xl">
          ...
          <div className="space-y-4 mt-1">
            <p className="text-sm text-slate-400">
              Joining as{" "}
              <span className="font-semibold text-amber-400">{nickname}</span>
            </p>
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[1, 2].map((t) => {
                  ...
                  const teamPlayers =
                    joinRoomPlayers?.filter((p) => p.team === t) ?? [];
                  return (
                    <button
                      key={t}
                      type="button"
                    ...
                    >
                      <div
                        className={`text-xs font-bold mb-2 ${t === 1 ? "text-rose-400" : "text-blue-400"}`}
                      >
                        Team {t}
                      </div>
        ...
        <DialogContent className="bg-[#0d1525]/95 backdrop-blur-xl border border-white/10 text-white max-w-sm shadow-2xl">
          ...
          <div className="space-y-4 mt-2">
            ...
            <Button
              className="w-full bg-gradient-to-br from-amber-400 to-amber-600 hover:from-amber-300 hover:to-amber-500 text-black font-bold h-11 border-0 shadow-[0_0_20px_rgba(251,191,36,0.3)] transition-all"
              disabled={
                !changeNicknameInput.trim() ||
                changeNicknameLoading ||
                changeNicknameInput.trim().length > 16
              }
      ...
      <Dialog open={nicknameDialogOpen} onOpenChange={() => {}}>
        <DialogContent
          hideCloseButton
          className="bg-[#0d1525]/95 backdrop-blur-xl border border-white/10 text-white max-w-sm shadow-2xl"
        >
          <DialogHeader>
            <DialogTitle className="text-xl font-bold bg-gradient-to-r from-amber-300 to-yellow-500 bg-clip-text text-transparent">
              Choose a Nickname
          ...
          <div className="space-y-4 mt-2">
            <p className="text-sm text-slate-400">
              Pick the name you'll use in all games.
            </p>
            <div>
              <Label className="text-slate-300 text-sm font-medium">
                Nickname
              ...
              <Input
                ...
                onKeyDown={async (e) => {
                  if (e.key === "Enter") {
                    ...
                    if (trimmed.length > 16) {
                      setNicknameError(
                        "Nickname must be 16 characters or fewer.",
                      );
                      return;
                    }
                    ...
                    if (taken) {
                      setNicknameError(
                        `"${trimmed}" is already taken. Pick another.`,
                      );
                      return;
                    }
                    setNickname(trimmed);
            ...
            <Button
              ...
              onClick={async () => {
                ...
                const trimmed = nickname.trim();
                if (trimmed.length > 16) {
                  setNicknameError("Nickname must be 16 characters or fewer.");
                  return;
                }
                setNicknameError(null);
                const taken = await checkNickname(trimmed, lockedNickname);
      ...
      <footer className="relative z-10 border-t border-white/5 mt-8 py-5">
        <div className="max-w-6xl mx-auto px-3 sm:px-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <p className="w-full text-[10px] text-slate-700 mt-1">
            Friendly Feud is an independent fan project inspired by classic TV
            survey game shows. "Family Feud" is a registered trademark of
            Fremantle. Friendly Feud is not affiliated with or endorsed by
            Fremantle.
          </p>
        </div>
      </footer>
